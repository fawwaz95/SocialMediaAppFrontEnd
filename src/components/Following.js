...
const Following = () => {
    ...
    const changeFollowingStatus = (current, index) => {
            ...
            {content.map((contentArray, index) => (
                <div className="hover:bg-sky-700 text-center rounded-xl relative flex justify-center items-center h-64"key={index}>
                    ...
                    <div className="absolute bottom-8 left-50 pb-4"> {contentArray.userName} </div>
                        <button className="absolute bottom-0 left-50 rounded-lg pl-4 pr-4 pt-1 pb-1 bg-slate-600 hover:bg-slate-400" onClick={() => {changeFollowingStatus(isFollowing, index)}}> 
                            {!isFollowing && userIndex === index ? <p>Following</p> : <p>Follow</p>}
                        </button>                          
                </div>
            ))}
        </div>
